...
import { isValidTokenProvider, isFunction } from './validations'

import { defaultTransformAuthorizationHeader, noop } from './defaults'
...
function createAuthorizedAxiosInstance({
  ...
  invariant(
    isFunction(transformAuthorizationHeader),
    'Invalid `transformAuthorizationHeader`. It expects a function!'
  )
  invariant(
    isFunction(refreshTokenRequest),
    'Invalid `getRefreshTokenRequest`. It expects a function!'
  )
  ...
  function setupInterceptors() {
    accessTokenRequestInterceptor = instance.interceptors.request.use(
      setAccessToken
    )
    invalidAccessTokenResponseInterceptor = instance.interceptors.response.use(
      null,
      checkInvalidAccessToken
    )
  }
  ...
  async function setAccessToken(requestConfig) {
    requestConfig.headers.Authorization = transformAuthorizationHeader(
      await tokenProvider.accessToken.get()
    )
  ...
  async function checkInvalidAccessToken(error) {
    if (error.response && error.response.status === invalidStatus) {
      try {
        removeInterceptors()
